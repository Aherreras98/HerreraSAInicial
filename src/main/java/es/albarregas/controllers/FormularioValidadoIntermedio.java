...
@WebServlet(name = "FormularioValidadoIntermedio", urlPatterns = {"/FormularioValidadoIntermedio"})
public class FormularioValidadoIntermedio extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("error") != null) {
            request.setAttribute("error", "Todos los campos con asterisco (*) son obligatorios.");
        ...
        if (nombre == null || nombre.trim().isEmpty()
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                out.println("<h1 class='titulo-principal'>Se han producido errores de entrada</h1>");
                out.println("<form action='FormularioValidadoIntermedio' method='post'>");
                out.println("<input type='hidden' name='nombre' value='" + (nombre != null ? nombre : "") + "'>");
                out.println("<input type='hidden' name='fechaNacimiento' value='" + (fechaNacimientoStr != null ? fechaNacimientoStr : "") + "'>");
                out.println("<input type='hidden' name='salario' value='" + (salario != null ? salario : "") + "'>");
                out.println("<input type='hidden' name='hijos' value='" + (hijos != null ? hijos : "") + "'>");
                if (preferencias != null) {
                    for (String preferencia : preferencias) {
                        out.println("<input type='hidden' name='preferencias' value='" + preferencia + "'>");
                    }
                }
                out.println("<input type='hidden' name='error' value='true'>");

                out.println("<div class='button'><button type='submit'>Aceptar</button></div>");
                out.println("</form>");
                out.println("</div>");
                out.println("</body>");
                out.println("</html>");
    ...
    private String pintarFormulario(HttpServletRequest request) {
        ...
        sb.append("<div class='container'>");
        sb.append("<form action='FormularioValidadoIntermedio' method='post'>");
        ...
        sb.append("<div class='button'><button type='submit' name='enviar'>Enviar Datos</button></div>");
        sb.append("</form>");
        sb.append("</div>");
    ...
    public String getServletInfo() {
        return "Servlet con validaciones de formulario del lado del servidor y pantalla intermedia de error.";
    }
}